...
/**
 * Stock-in (receiving) form. Admin-only; the repository enforces that independently of this
...
fun StockInScreen(
    ...
    // The scanner destination writes its result onto this entry's SavedStateHandle and pops.
    ...
    Scaffold(
        ...
        if (warehouses.isEmpty()) {
            // Receiving needs somewhere to put the goods.
            EmptyState(
        ...
        Column(
            ...
            if (state.productId != null) {
                BalancePreview(
                    ...
                    resulting = state.resultingBalance,
                    unit = state.unit
                )
            }

            QuantityField(
                label = stringResource(R.string.quantity_received),
            ...
            LabeledDropdown(
                fieldLabel = stringResource(R.string.supplier),
                selected = suppliers.firstOrNull { it.id == state.supplierId },
                items = suppliers,
                label = { it.name },
                secondaryLabel = { it.phone },
                onSelected = { viewModel.onSupplierSelected(it.id) },
                placeholder = stringResource(R.string.select_supplier)
            )
            ...
            Button(
                ...
                if (state.submitting) {
                    ...
                    Text(
                        stringResource(R.string.confirm_stock_in),
